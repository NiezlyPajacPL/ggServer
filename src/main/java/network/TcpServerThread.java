...
public class TcpServerThread implements Server {
    ...
    public TcpServerThread(Socket clientSocket, SubtitlesPrinter subtitlesPrinter, Map<ConnectionData, PrintWriter> tcpUsers) {
    ...
    public void sendPacket(Packet packetToSend) {
        output = tcpUsers.get(packetToSend.getConnectionData());
    ...
    private void registerUser(Registration registration) {
        ...
        try {
            ...
            if (!dataBaseManager.clientExistInDB(registration.name) && registration.name != null) {
                ...
                tcpUsers.put(new ConnectionData(registration.inetAddress,registration.port),output);
